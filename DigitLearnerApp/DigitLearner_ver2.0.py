...
HISTORY_FILE = os.path.join(BASE_DIR, "history.csv")  # 履歴ファイルのパス
...
class DigitLearnerApp:
    ...
    def __init__(self, root):
        # ウィンドウ設定
        self.root = root
        self.root.title("手書き数字認識アプリ(ver2.0)")
        ...
        btn_frame.pack()
        self.predict_btn = tk.Button(btn_frame, text="予想", command=self.predict_canvas, width=10)
        ...
        self.add_btn = tk.Button(correct_frame, text="正解を学習", command=self.add_training_data)
        ...
        self.reset_btn = tk.Button(reset_frame, text="学習データを初期化", command=self.reset_data, width=20)
        self.reset_btn.pack()

        # 履歴CSV出力ボタンの作成
        export_frame = tk.Frame(root)
        export_frame.pack(pady=10)
        self.export_btn = tk.Button(export_frame, text="学習履歴をCSV出力", command=self.export_history, width=20)
    ...
    # 学習データを初期状態に戻す
    def reset_data(self):
        if mb.askyesno("確認", "本当に学習データを初期化しますか？"):
            if os.path.exists(DATA_FILE):
                os.remove(DATA_FILE)
            self.X = self.scaler.fit_transform(self.digits.data)
            self.y = self.digits.target
            self.model = self.train_model()
            mb.showinfo("初期化完了", "学習データを初期状態に戻しました。")

    # 学習履歴をCSV出力
    def export_history(self):
        if not self.history:
            mb.showinfo("情報", "履歴データがありません。")
            return
        with open(HISTORY_FILE, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["No", "日時", "予想値", "正解値", "確信度(%)", "正否(1=正解,0=不正解)"])
            writer.writerows(self.history)
        mb.showinfo("完了", f"学習履歴をCSVとして出力しました。\n{HISTORY_FILE}")
