...
class DigitLearnerApp:
    ...
    def __init__(self, root):
        # ウィンドウ設定
        self.root = root
        self.root.title("Digit Learner App(ver3.0)")
        ...
        # データ番号表示
        self.data_number_label = tk.Label(root, text=f"Current Data No: {self.history_count}", font=("Helvetica", 12))
        self.data_number_label.pack(pady=5)
        ...
        # 学習中断・再開ボタン
        pause_resume_frame = tk.Frame(root)
        pause_resume_frame.pack(pady=10)
        self.pause_btn = tk.Button(pause_resume_frame, text="Pause Learning", command=self.pause_learning, width=15)
        self.resume_btn = tk.Button(pause_resume_frame, text="Resume Learning", command=self.resume_learning, width=15)
        ...
        # CSVからインポートして学習
        self.import_btn = tk.Button(root, text="Import & Learn from CSV", command=self.import_csv_and_learn, width=25)
    ...
    def export_history(self):
        ...
        header = ["No", "datetime", "predicted", "correct", "confidence(%)", "is_correct"]
        header += [f"vec_{i}" for i in range(64)]
    ...
    def add_training_data(self):
        ...
        if label.isdigit():
            ...
            if not self.learning_paused:
                ...
                self.model = self.train_model()
                self.save_data()
            mb.showinfo("Learned", f"Data No.{self.history_count} recorded.")
            self.history_count += 1
            self.data_number_label.configure(text=f"Current Data No: {self.history_count}")
    ...
    def pause_learning(self):
        ...
        mb.showinfo("Pause", "Learning has been paused.")

    # CSVファイルから学習データをインポート
    def import_csv_and_learn(self):
        ...
        try:
            ...
            vec_cols = [f"vec_{i}" for i in range(64)]
            X_new = df[vec_cols].astype(float).values
            y_new = df["correct"].astype(int).values
            X_scaled = self.scaler.transform(X_new)
            self.X = np.vstack([self.X, X_scaled])
            self.y = np.append(self.y, y_new)
            self.model = self.train_model()
            self.save_data()
            mb.showinfo("Success", f"{len(X_new)} records imported and learned.")
        except Exception as e:
            mb.showerror("Error", f"Failed to import CSV:\n{str(e)}")

    # 学習を再開
    def resume_learning(self):
        self.learning_paused = False
        mb.showinfo("Resume", "Learning has been resumed.")
